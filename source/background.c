...
)
{
  ...
  if (pba->has_NEDE_trigger == _TRUE_)
  {
    if ((pba->has_NEDE_trigger_DM == _FALSE_) || (a_rel < pba->a_trigger_fluid))
    {
      /** The contribution of the trigger field was not added to dp_dloga, add p_prime_trigger here: */
    ...
    struct background *pba)
{
  ...
  class_define_index(pba->index_bi_rho_trigger_fld, pba->has_NEDE_trigger_DM, index_bi, 1);
    ...
    struct background *pba)
{
  ...
  /*reset deycay_flag before integration loop starts, decay_flag == _FALSE_ means that the decay has not yet taken place.*/
  ...
  while (pvecback_integration[pba->index_bi_a] < pba->a_today)
  {
    ...
    /*New EDE*/ /* Here we are inside the loop of the background integration, this is hence called at every time step*/ /* stop1 */
    ...
    if (pba->has_NEDE == _TRUE_)
    {
      ...
      /*Update: We now allow for a more sizable amount of potential trigger energy, so we need to track it.*/

      if (pba->has_NEDE_trigger_DM == _FALSE_)
      {
      ...
      else if ((pba->NEDE_trigger_mass * pba->Trigger_fluid_H_over_m > pvecback[pba->index_bg_H] * 1.05) && (pba->trigger_fluid_flag == _TRUE_))
      {
      ...
      /*New EDE: If the trigger field makes a sizeable contribution to DM we have to make sure that either the fluid approximation is used or the integration step size is adapted.*/
      if ((pba->has_NEDE_trigger_DM == _TRUE_) && (pba->trigger_fluid_flag == _FALSE_))
      {
        ...
        if (pvecback[pba->index_bg_H] < pba->Trigger_fluid_H_over_m * pba->NEDE_trigger_mass)
        {
          ...
          pba->H_fluid = pvecback[pba->index_bg_H];
          pba->H_prime_fluid = pvecback[pba->index_bg_H_prime];
      ...
      // In the case where we track trigger oscillations, we have to make sure that the integration is accurate enough. Here we adapt the integration step-size correspondingly.
...
)
{
  ...
  */
  ...
  if (pba->has_NEDE_trigger == _TRUE_ && pba->has_NEDE == _TRUE_)
  {
    ...
    if (pba->has_NEDE_trigger_DM == _TRUE_)
    {
      pvecback_integration[pba->index_bi_rho_trigger_fld] = 0.0;
    }
  }
    ...
    ErrorMsg error_message)
{
  ...
  if (pba->has_NEDE_trigger == _TRUE_) // stop2
  {
    /** - Trigger scalar field equation: \f$ \phi'' + 2 a H \phi' + a^2 dV = 0 \f$  (note H is wrt cosmic time) */
    ...
    struct background *pba)
{
  ...
  if (pba->background_verbose > 1)
  {
    ...
    if (pba->has_NEDE_trigger_DM)
    {
      _class_print_species_("New EDE trigger", trigger);
      budget_matter += pba->Omega0_trigger;
    }
